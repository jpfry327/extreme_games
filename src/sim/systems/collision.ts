...
/** Circle overlap between a projectile and a ship, using the ship's configured
 *  collision radius and the projectile's own radius. */
function overlaps(p: Projectile, target: Player): boolean {
  const reach = p.radius + shipConfig(target.shipType).radius;
  const dx = p.x - target.kinematics.x;
  const dy = p.y - target.kinematics.y;
